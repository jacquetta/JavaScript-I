...
let lastCar = [];
   for(let i = 0; i < inventory.length; i++){
    if(inventory[i].id === findCar){
       lastCar.push(inventory[i].id);
...
console.log(`Car ${lastCar[0]} is a ${lastCar[3]} ${lastCar[1]} ${lastCar[2]} `);


// ==== Challenge 2 ====
// The dealer needs the information on the last car in their inventory.  What is the make and model of the last car in the inventory?  Log the make and model into the console.
let inventoryLastCar = [];
let lastPlace = inventory.length;
for(let i = 0; i < inventory.length; i++){

    if(inventory[i].id === lastPlace){
        inventoryLastCar.push(inventory[i].car_make);
        inventoryLastCar.push(inventory[i].car_model);
    }
}

console.log(`${inventoryLastCar[0]} ${inventoryLastCar[1]}`);
...
// ==== Challenge 4 ====
// The accounting team needs all the years from every car on the lot. Create a new array from the dealer data containing only the car years and log the result in the console.car
let carYears = [];
for (let i = 0; i < inventory.length; i++) {
...
for(let i = 0; i < inventory.length; i++){
    if (inventory[i].car_make === bmw || inventory[i].car_make === audi) {
     ...
     carJSON = JSON.stringify(BMWAndAudi);
}
